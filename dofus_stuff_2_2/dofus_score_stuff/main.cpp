...
int main()
{
	...
	if (!true)
	{
		...
		DebugPosItem("Heaume Tapodas", items_type, t1, p1);
		DebugPosItem("Cape d'Ogivol", items_type, t2, p2);

		Citem test(items_type[t1][p1]);
		test += items_type[t2][p2];
		std::cout << test.Gtexte_valeurs() << "\n";
		return 1;
	}
	...
	if (GetIntSec("0 pour enlever les items inutiles, 1 sinon : ", 0, 1) == 0)
	{
		...
		std::cout << "\n";
		int PA_min_voulu{ 0 }, PM_min_voulu{ 0 };
		PA_min_voulu = GetInt("retirer tous les items qui ne permettent pas d'atteindre : PA = ");
		PM_min_voulu = GetInt("retirer tous les items qui ne permettent pas d'atteindre : PM = ");
		//DebugChercheItem("Heaume Tapodas", items_type);
		//Pause();

		ClearItemsPA_PM(items_type, PA_min_voulu, PM_min_voulu);
